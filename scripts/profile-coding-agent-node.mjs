...
const agentDirEnvName = "SUMMON_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "SUMMON_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific SUMMON_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force SUMMON_OFFLINE=1 / SUMMON_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.SUMMON_OFFLINE = "1";
		env.SUMMON_SKIP_VERSION_CHECK = "1";
	}
	return env;
}

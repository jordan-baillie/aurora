...
```bash
npm install @summon/ai
```

TypeBox exports are re-exported from `@summon/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@summon/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@summon/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@summon/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@summon/ai';
...
```typescript
import { getImageModel, generateImages } from '@summon/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@summon/ai';
...
```typescript
import { getModel, stream } from '@summon/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@summon/ai';
...
```typescript
import { Model, stream } from '@summon/ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Summon thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@summon/ai';
...
```typescript
import { getModel, complete, Context } from '@summon/ai';
...
```typescript
import { Context, getModel, complete } from '@summon/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@summon/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@summon/ai';
...
```bash
npx @summon/ai login              # interactive provider selection
npx @summon/ai login anthropic    # login to specific provider
npx @summon/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@summon/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@summon/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@summon/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@summon/ai`

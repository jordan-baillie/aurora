...
// Pure: pick a transport for one task in a batch. An explicit override always wins; otherwise use the
// pool only when there are ≥POOL_MIN_BATCH tasks of this task's own bundle in the batch.
export function pickTransport(sameBundleCount: number, override?: string): "oneshot" | "pool" {
	if (override === "pool" || override === "oneshot") return override;
	return sameBundleCount >= POOL_MIN_BATCH ? "pool" : "oneshot";
}

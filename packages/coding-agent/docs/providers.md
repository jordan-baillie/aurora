...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/jordan-baillie/summon/blob/main/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/jordan-baillie/summon/blob/main/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.summon/agent/auth.json`:
...
```bash
summon --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
summon --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
summon --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal summon usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
summon --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Summon automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.

 ...
 * - ~/.summon/agent/presets.json (global)
 * - <cwd>/.summon/presets.json (project-local)
 *
...
import type { Api, Model } from "@summon/ai";
import type { ExtensionAPI, ExtensionContext } from "@summon/coding-agent";
import { DynamicBorder, getAgentDir } from "@summon/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@summon/tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".summon", "presets.json");
...
export default function presetExtension(summon: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: summon.getThinkingLevel(),
				tools: summon.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			summon.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = summon.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				summon.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.summon/agent/presets.json or .summon/presets.json",
				"warning",
			);
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await summon.setModel(originalState.model);
				}
				summon.setThinkingLevel(originalState.thinkingLevel);
				summon.setActiveTools(originalState.tools);
			} else {
				summon.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	summon.registerShortcut(Key.ctrlShift("u"), {
	...
	summon.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = summon.getFlag("preset");
	...
	summon.on("turn_start", async () => {
		if (activePresetName) {
			summon.appendEntry("preset-state", { name: activePresetName });
		}
	});
}

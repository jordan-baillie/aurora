import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "@summon/coding-agent";
import type { Component, EditorTheme, TUI } from "@summon/tui";
...
export default function (summon: ExtensionAPI) {
	...
	summon.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await summon.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = summon.getThinkingLevel();

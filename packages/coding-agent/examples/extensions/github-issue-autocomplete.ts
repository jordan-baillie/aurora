...
async function resolveGitHubRepo(summon: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await summon.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (summon: ExtensionAPI): void {
	summon.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(summon, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await summon.exec(
					"gh",
					[
						"issue",
						"list",

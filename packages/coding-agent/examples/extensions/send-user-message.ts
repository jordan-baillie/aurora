 ...
 *
 * Demonstrates summon.sendUserMessage() for sending user messages from extensions.
 * Unlike summon.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (summon: ExtensionAPI) {
	...
	summon.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			summon.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	summon.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				summon.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	summon.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				summon.sendUserMessage(args, { deliverAs: "followUp" });
	...
	summon.registerCommand("askwith", {

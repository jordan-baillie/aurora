...
export default function (summon: ExtensionAPI) {
	...
	summon.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "summon RPC Demo (new session)" : "summon RPC Demo");
	...
	summon.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	summon.on("turn_end", async (_event, ctx) => {
	...
	summon.registerCommand("rpc-input", {
	...
	summon.registerCommand("rpc-editor", {
	...
	summon.registerCommand("rpc-prefill", {

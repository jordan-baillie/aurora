...
describe("package commands", () => {
	...
	beforeEach(() => {
		...
		originalPiPackageDir = process.env.SUMMON_PACKAGE_DIR;
		originalExitCode = process.exitCode;
		originalExecPath = process.execPath;
		originalVersionCheckUrl = process.env.SUMMON_VERSION_CHECK_URL;
		process.exitCode = undefined;
		process.env[ENV_AGENT_DIR] = agentDir;
		// Self-update consults the (opt-in) version-check endpoint; configure it so those paths run.
		process.env.SUMMON_VERSION_CHECK_URL = "https://updates.example/latest-version";
	...
	afterEach(() => {
		...
		if (originalPiPackageDir === undefined) {
			delete process.env.SUMMON_PACKAGE_DIR;
		} else {
			process.env.SUMMON_PACKAGE_DIR = originalPiPackageDir;
		}
		Object.defineProperty(process, "execPath", { value: originalExecPath, configurable: true });
		if (originalVersionCheckUrl === undefined) {
			delete process.env.SUMMON_VERSION_CHECK_URL;
		} else {
			process.env.SUMMON_VERSION_CHECK_URL = originalVersionCheckUrl;
		}
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("summon install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "summon --help" or "summon install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: summon install <source> [-l]");
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@summon", "coding-agent");
...
`,
	...
	it("uses the current package name when the update check omits packageName", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		writeFileSync(

...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@summon");
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@summon+coding-agent@0.67.68\\node_modules\\@summon\\coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@summon/coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts @summon/coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@summon/coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@summon/coding-agent")).toBe(
			"Update @summon/coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@summon/coding-agent");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "@summon/coding-agent"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts @summon/coding-agent`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@mariozechner/coding-agent", undefined, "@new-scope/pi");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "@new-scope/pi"],
			display: `npm --prefix ${prefix} uninstall -g @mariozechner/coding-agent && npm --prefix ${prefix} install -g --ignore-scripts @new-scope/pi`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@mariozechner/coding-agent"],
					display: `npm --prefix ${prefix} uninstall -g @mariozechner/coding-agent`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "@new-scope/pi"],
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@summon/coding-agent", ["npm", "--prefix", prefix]);
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "--ignore-scripts", "@summon/coding-agent"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g --ignore-scripts @summon/coding-agent`);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@summon\\coding-agent";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@summon/coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts @summon/coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "@summon/coding-agent"],
			display: "bun install -g --ignore-scripts @summon/coding-agent",
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display: "pnpm remove -g @mariozechner/coding-agent && pnpm install -g --ignore-scripts @new-scope/pi",
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@mariozechner/coding-agent"],
					display: "pnpm remove -g @mariozechner/coding-agent",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "@new-scope/pi"],
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@summon/coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@summon", "coding-agent");
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display: "yarn global remove @mariozechner/coding-agent && yarn global add --ignore-scripts @new-scope/pi",
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@mariozechner/coding-agent"],
					display: "yarn global remove @mariozechner/coding-agent",
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/pi"],
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "@new-scope/pi"],
			display: "bun uninstall -g @mariozechner/coding-agent && bun install -g --ignore-scripts @new-scope/pi",
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@mariozechner/coding-agent"],
					display: "bun uninstall -g @mariozechner/coding-agent",
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "@new-scope/pi"],
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@summon/coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@summon/coding-agent")).toContain("the install path is not writable");
	});
});

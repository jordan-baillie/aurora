...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@summon/ai";
import { getApiProvider } from "@summon/ai";
import { getOAuthProvider } from "@summon/ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `summon-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);

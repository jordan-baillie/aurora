...
- Aurora harness: **orchestration mode** (`SUMMON_ORCHESTRATION=off|auto|ultra`, default `auto`; toggle live with `/orchestrate`) — closes the gap where the spawn tools were live on the main agent but nothing told it *when* to use them. While the mode is on, a `before_agent_start` handler injects a delegate-by-default doctrine — plus the live specialist roster and saved-recipe catalog — into the resident session's system prompt every turn, so substantial work fans out to sub-agents instead of being done alone. `auto` delegates for substantial/parallelisable work and stays solo for trivial edits; `ultra` is a standing opt-in to decompose + fan out + adversarially verify every substantial task; `off` restores the base prompt.
- Aurora harness: **recipe discoverability** — `run_team` / `run_blueprint` tool descriptions now embed the loaded team/blueprint catalog (name + description) the same way the registry digest is injected, so a model can reach for a saved recipe by name instead of only learning the names from a failed-lookup error.
- Aurora harness: **`orchestrate({ goal })`** — the one-call plan+run+verify primitive. Synthesises a validated DAG from a natural-language goal (frontier planner) and RUNS it in one call: scoped specialist **agent** nodes execute inline and in parallel; deterministic **code/shell** nodes are force-gated and **pause for human approval** (`approve_gate`/`resume_run`) so nothing irreversible runs unattended. Durable + resumable. Unlike `plan_and_run` (preview-by-default, gated behind `HARNESS_PLAN_RUN`), `orchestrate` always runs the safe parts; pass `dry_run:true` to preview. The orchestration doctrine now points open-ended goals here.
- Aurora harness: **governor back-pressure to the model** — spawn/fan-out tool results append a one-line governor hint (`formatGovernorHint`) when a hard token budget is configured (`HARNESS_WINDOW_TOKENS>0`) and the rolling window is hot, so the resident agent can self-regulate fan-out width. No-op in the default config (no budget ⇒ `windowPct` 0 ⇒ no hint), so it never discourages wide fan-out.

### Changed

- Aurora harness: **multi-reviewer adversarial verification** — `spawn_agent`'s `review` now fans out to N independent reviewers (`reviewers` param; odd default 3, or 5 in `ultra`, capped by `HARNESS_REVIEWERS_MAX`) each focused on a distinct lens (correctness / regressions & safety / tests & edge-cases), with a strict-majority, fail-closed verdict (`aggregateReviews`/`runWithReviewers`). Replaces the single hardcoded reviewer so a substantial change is cross-checked from multiple angles rather than trusting one opinion; odd counts keep the majority meaningful (a single diverse-lens dissent doesn't sink a good change).

...
# Summon Observability Design Notes

## Goal

Make `packages/ai` and `packages/agent`/harness observable without depending on OpenTelemetry, Sentry, or any APM vendor.

Summon should emit stable, structured lifecycle events. External listeners can convert those events into OTel spans, Sentry spans, logs, metrics, or custom telemetry.
...
```text
traceId=t1 spanId=s1 parent=-  name=summon.agent.prompt
traceId=t1 spanId=s2 parent=s1 name=summon.agent.turn
traceId=t1 spanId=s3 parent=s2 name=summon.ai.provider.request
traceId=t1 spanId=s4 parent=s2 name=summon.agent.tool_call
traceId=t1 spanId=s5 parent=s4 name=summon.session.append_entry
```
...
```ts
await Promise.all([
  runWithSummonContext({ userId: "alice" }, () => harness.prompt("A")),
  runWithSummonContext({ userId: "bob" }, () => harness.prompt("B")),
]);
```

Deep code can then read the correct current context for the active async chain.

Summon must run in Node, Bun, browser, workers, and other JS runtimes, so ALS cannot be the core abstraction. It should be a runtime adapter.

## Core design

Summon owns a small runtime-agnostic observability abstraction:

```ts
export interface SummonObservabilityContext {
  traceId?: string;
  currentSpanId?: string;
  userContext?: Record<string, unknown>;
}

export interface SummonObservabilityEvent {
...
export interface SummonObservability {
  getContext(): SummonObservabilityContext | undefined;
  runWithContext<T>(context: SummonObservabilityContext, fn: () => T): T;
  emit(event: SummonObservabilityEvent): void;
...
```ts
export function configureSummonObservability(observability: SummonObservability): void;
export function subscribeSummonObservability(listener: (event: SummonObservabilityEvent) => void): () => void;
export function runWithSummonContext<T>(userContext: Record<string, unknown>, fn: () => T): T;
...
channel("summon.observability").publish(event);
```

Subscribers can create OTel/Sentry spans without monkey-patching summon.

## What summon emits

Summon emits what happened. It does not create OTel/Sentry spans directly.
...
summon.agent.prompt
summon.agent.skill
summon.agent.prompt_template
summon.agent.compaction
summon.agent.branch_navigation
summon.agent.session.append_entry
summon.ai.provider.request
```
...
```text
summon.agent.turn
summon.agent.tool_call
summon.agent.queue_update
summon.ai.provider.retry
summon.ai.provider.first_token
summon.ai.provider.usage
summon.session.read
summon.session.write
```
...
Observability must never affect summon execution.
...
{
  type: "start",
  name: "summon.ai.provider.request",
  traceId: "t1",
  spanId: "s3",
  parentSpanId: "s1",
  context: {
...
```text
packages/ai
  emits summon.ai.* events

packages/agent
  emits summon.agent.* / summon.session.* events
```
...
packages/otel
  subscribes to summon events and creates OpenTelemetry spans
```

## Thesis

Summon defines a stable, safe event contract. Adapters define where events go.

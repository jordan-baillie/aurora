...
Stateful agent with tool execution and event streaming. Built on `@summon/ai`. Part of Summon, which is built on the Pi coding agent by Mario Zechner.

## Installation

```bash
npm install @summon/agent-core
```
...
```typescript
declare module "@summon/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@summon/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@summon/agent-core";

...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.SUMMON_HARDWARE_CURSOR === "1";
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.SUMMON_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".summon", "agent", "pi-debug.log");
		...
		// Configurable via setClearOnShrink() or SUMMON_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".summon", "agent", "pi-crash.log");
				const crashData = [
		...
		if (process.env.SUMMON_TUI_DEBUG === "1") {
